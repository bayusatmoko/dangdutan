...
export default function Home() {
  ...
  const [currentSong, setCurrentSong] = useState('https://songssongs.s3.ap-southeast-1.amazonaws.com/nadhif.mp3')
  ...
  const renderView = () => {
    return songs.map((song, index) => (
      <div onClick={() => {
        console.log(`${S3_BASE}/${song.audioUrl}`);
        setCurrentSong(`${S3_BASE}/${song.audioUrl}`)}
        
        } key={`${song}-${index}`} className="flex w-1/4 flex-wrap m-2">
